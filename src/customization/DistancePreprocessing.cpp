...
void DistancePreprocessing::precomputeDistances(uint32_t preprocessingParameter) {
    ...
    for (uint32_t node = G->numVertices(); node-- > 0;) {
        ...
        if (node == 8230481 ) {
            ...
            for (auto pn : precomputedNodes[node]) {
                std::cout << pn << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262286 precomputed Nodes: ";
            for (auto pn : precomputedNodes[16262286]) {
                std::cout << pn << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262337 precomputed Nodes: ";
            for (auto pn : precomputedNodes[16262337]) {
                std::cout << pn << ", ";
            }
            std::cout << std::endl;

            std::cout << "Node 8230481 precomputed distances up: ";
            for (auto dist : precomputedDistancesUp[node]) {
                std::cout << dist << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262286 precomputed distances up: ";
            for (auto dist : precomputedDistancesUp[16262286]) {
                std::cout << dist << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262337 precomputed distances up: ";
            for (auto dist : precomputedDistancesUp[16262337]) {
                std::cout << dist << ", ";
            }
            std::cout << std::endl;

            std::cout << "Node 8230481 successorUp: ";
            for (auto succ : successorUp[node]) {
                std::cout << succ << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262286 successorUp: ";
            for (auto succ : successorUp[16262286]) {
                std::cout << succ << ", ";
            }
            std::cout << std::endl;
            std::cout << "Node 16262337 successorUp: ";
            for (auto succ : successorUp[16262337]) {
                std::cout << succ << ", ";
            }
            std::cout << std::endl;
        }
...
void DistancePreprocessing::selectNodesWithMaxDistanceToRoot(uint32_t maxDistance) {
    for (uint32_t node = G->numVertices(); node-- > 0;) {
        ...
        precomputedNodes[node] = precomputedNodes[G->parentOf(node)];
        if (precomputedNodes[G->parentOf(node)].size() < maxDistance) precomputedNodes[node].push_back(G->parentOf(node));
    }
}
