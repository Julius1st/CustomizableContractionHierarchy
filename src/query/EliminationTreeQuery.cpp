    ...
    for (uint32_t sIndex = 0; sIndex < G->precomputedNodes[s].size(); sIndex++) {
        ...
        while (currentVertex != G->precomputedNodes[s][sIndex]) {
            if (precNodesIndex < G->precomputedNodes[currentVertex].size() && G->precomputedNodes[currentVertex][precNodesIndex] == G->precomputedNodes[s][sIndex]) {
                if (precNodesIndex >= G->successorUp[currentVertex].size()) std::cout << "precNodesIndex out of bounds!" << std::endl;
                if (G->successorUp[currentVertex][precNodesIndex] >= G->numVertices()) std::cout << "successorUp out of bounds! " << G->successorUp[currentVertex][precNodesIndex] << " currentVertex: " << currentVertex << std::endl;
